...
class PWCNet(nn.Module):
    ...
    """
    def __init__(self, md=4,requires_grad=False,is_train=False):
        """
...
class PWCNet_old(nn.Module):
    """
    PWC-DC net. add dilation convolution and densenet connections

    """
    def __init__(self, md=4,is_train=False):
        """
        input: md --- maximum displacement (for correlation. default: 4), after warpping

        """
        super(PWCNet_old,self).__init__()
        self.is_train=is_train
        self.conv1a  = conv(3,   16, kernel_size=3, stride=2)
        self.conv1b  = conv(16,  16, kernel_size=3, stride=1)
        self.conv2a  = conv(16,  32, kernel_size=3, stride=2)
...
def pwc_dc_net(path=None):
    ...
    model = PWCNet(requires_grad=False)
    if path is not None:
